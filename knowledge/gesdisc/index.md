...
- [GES DISC subsetting and OPeNDAP access to MERRA-2, AIRS and the OCO SIF Lite files: Cloud OPeNDAP by granule URL with a DAP4 constraint, the Harmony enterprise subsetter by collection, bounding box, variables and time, and an Earthdata Login bearer token on every data request; the on-premises subsetter and OPeNDAP servers retire in September 2026](connectors/gesdisc-subsetter-opendap.md), status: draft
