...
- [A MOD13 16-day composite is not an image of one day: each pixel is the observation the constrained-view maximum value rule selected, its date is in the composite day of the year layer, adjacent pixels can come from different days and geometries, and the date in the file name is the period, not the observation](gotchas/composite-day-of-year-layer.md), severity medium, status: draft
