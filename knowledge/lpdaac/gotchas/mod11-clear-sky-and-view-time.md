...
Clear_sky_days and Clear_sky_nights recording how many contributed;
...
value in the same layer, with the count in another. The daytime and
...
anomaly series from MOD11A2 mixes changes in Clear_sky_days with
changes in the surface. A comparison against a station record at a
...
of this raises an error: the fill is a number, the count layer is
optional to read, and the time layer looks like a constant to a
script that never opens it.
...
the daily product or Clear_sky_days and Clear_sky_nights in the
eight-day product, as a weight or as a minimum count for a cell to be
reported. A comparison with a timed record uses the cell's own
...
over the tile spans a range of hours rather than one value, and
Clear_sky_days on the eight-day tile takes several values across the
tile in any cloudy season.[^user-guide][^a2-page] A mean computed with
