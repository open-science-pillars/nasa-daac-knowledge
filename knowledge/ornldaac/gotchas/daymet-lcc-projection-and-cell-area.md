...
# area-weighted total is a few percent over most of the domain and
# larger at its northern edge, and a plotted field is visibly off when
...
sources:
  ...
  - id: daymet-web-services
    ...
    title: "Daymet project site, web services page: the note that the netCDF subset service finds the minimum bounding area of a lat/lon box in the projected system, so the output is square in the projection"
  - id: thornton-2021
...
a snow water mass, a radiation load) carries the areal scale factor as
a bias: about 10 percent high at the latitude of origin, several
percent low in the tropics and at 70 N, and roughly half at the
northern edge, with the sign changing across the standard parallels,
so the bias differs between two regions being compared. A lat/lon
