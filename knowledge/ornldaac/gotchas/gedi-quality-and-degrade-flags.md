...
applied models represent, and its definition is l2a_quality_flag set,
urban_proportion below 50, is_land set, a prediction stratum
assigned, leaf_off_flag not set unless the model's only predictor is
RH98, and RH100 below 150 m; the guide's example is that the
...
of higher systematic geolocation error, and it is meant to be used
...
reverse, silently selects nothing or everything depending on how the
missing name is handled.[^ornl-l4a-v3-guide] A beam sensitivity
...
and elev_highestreturn_outlier_flag, and this is the sample L4B is
built on.[^ornl-l4a-v3-guide][^ornl-l4b-v2-guide]
