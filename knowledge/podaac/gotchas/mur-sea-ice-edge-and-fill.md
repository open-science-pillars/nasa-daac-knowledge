...
title: "MUR at the sea ice edge: the field is gap-free under ice too, where the value comes from a high-latitude parameterization driven by the ice concentration rather than from SST observations, and only the mask and sea_ice_fraction fields say which pixels those are"
description: "A GHRSST Level 4 file has a value in every ocean pixel and a fill value only on land, so MUR carries an analysed_sst under sea ice. The producer states that the OSI SAF ice concentration is used for an improved SST parameterization at high latitudes, so the value under ice is set by the ice product and that parameterization, not by a retrieval. The ice-covered pixels are identified by the sea_ice bit of the mask and by sea_ice_fraction, both from the ice product, and the set of such pixels changes daily with the ice. A polar box mean, an ice-edge gradient, a marginal-ice-zone time series or a trend in a seasonally ice-covered region that reads analysed_sst without those two fields mixes parameterized values with analysed ones and carries the ice product's decisions as ocean temperature."
...
sources:
  ...
  - id: validity-domain
    ...
    title: "This bundle's draft validity domain for MUR, which quotes the analysis paper's statement that SST analysis products disagree with each other in the summer Arctic and caps the supported domain at 66N for that reason"
  - id: dataset
    resource: ../datasets/ghrsst-mur.md
    title: "This bundle's MUR dataset concept: the mask and sea_ice_fraction fields, and analysis_error largest at the ice edge"
---
...
ice-covered pixels. The producer states that the ice concentration
data come from the EUMETSAT OSI SAF High Latitude Processing Center
and are also used for an improved SST parameterization for the high
latitudes, which is the product's own account of where the value
under ice comes from: a parameterization driven by the ice product,
not a retrieval of the water
temperature.[^podaac-collection] The temperature that
parameterization assigns is not stated on the collection page, the
project page or the specification, so the value under ice is read
from the file, not from the documentation. Two fields identify the
pixels concerned. The mask carries a sea_ice bit (bit 3 in the
specification's flag_masks, beside water, land and the optional lake
and river bits), and sea_ice_fraction carries the ice area fraction
...
alone averages parameterized values in the ice-covered pixels with
...
edge is the transition between an analysed value and a parameterized
...
against a climatology that includes the parameterized values is
...
those pixels, since the documentation read here gives the mechanism
but not the number.[^podaac-collection] The bundle's validity domain
...
ice concentration source and its use in the high-latitude
parameterization, and its variable table shows mask and
...
fill temperature under ice is reported here as undocumented rather
than quoted.
