...
- [MUR at the sea ice edge: the field is gap-free under ice too, where no SST retrieval exists and the analysed value is not an observation, and only the mask and sea_ice_fraction fields say which pixels those are](gotchas/mur-sea-ice-edge-and-fill.md), severity medium, status: draft
...
- [MUR near-real-time and retrospective files share one collection and are told apart only by a global attribute: the one-day and four-day analyses of the same date can differ in value, so a series across the latency boundary changes file kind without saying so](gotchas/mur-nrt-versus-final.md), severity low, status: draft
