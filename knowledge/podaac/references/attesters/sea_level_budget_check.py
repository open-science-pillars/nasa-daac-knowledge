...
from trend_recompute import (CONFIDENCE, MIN_DOF, MIN_MONTHS, REL_TOL,
                             check_block, close, default_deseasonalize,
                             t_quantile)

HERE = Path(__file__).resolve().parent
DEFAULT_COMPUTATION = HERE.parent / "computations" / "sea_level_budget.py"
...
def attest(receipt_path: Path, computation: Path):
    ...
    if series_ok:
        ...
        method_path = computation.parent / "ecco_trend_ci.py"
        problems = []
...
def selftest(computation: Path) -> int:
    ...
    with tempfile.TemporaryDirectory() as d:
        ...
        (w / "ecco_trend_ci.py").write_bytes((computation.parent / "ecco_trend_ci.py").read_bytes())
